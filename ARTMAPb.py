...
class ArtFun:
    ...
    def DistArt(self, input):
        size = self.Art.bot_size[0] * self.Art.bot_size[1]
        self.Art.bot.top_stm = np.zeros(self.Art.bot_size[2])
        self.Art.top.top_stm = np.zeros(self.Art.top_size[2])
        signal = self.BotUnChoc(input)
        number = np.count_nonzero(self.Art.bot.committ)

        # compute distributed code for bot object category
        thresh_set = [signal[j][0] for j in range(number) if signal[j][1] > self.Art.alpha * size]
        match_set = [j for j in thresh_set if signal[j][1] == size]
        non_match_set = [j for j in thresh_set if j not in match_set]
        for j in match_set:
            self.Art.bot.top_stm[j] = 1 / len(match_set)

        for j in non_match_set:
            numer = self.weight(signal[j][1])
            denom = sum(self.weight(signal[k][1]) for k in non_match_set)
            self.Art.bot.top_stm[j] = numer / denom

        prediction = np.zeros(self.Art.top_size[2])

        for k in range(self.Art.top_size[2]):
            prediction[k] = sum(self.Art.bot.top_stm[j] * self.Art.mid.crs_ltm[j][k] for j in range(number))

        # predict top object category from distributed code of bot object category
        index_max = np.unravel_index(np.argmax(prediction), prediction.shape)
        self.Art.top.top_stm[index_max] = 1.0


    def TestArt(self, top_input, bot_input):
        size = self.Art.bot_size[0] * self.Art.bot_size[1]
        learned_nodes = np.count_nonzero(self.Art.bot.committ)
        learned_weights = self.Art.bot.top_ltm
        choice_signal = np.zeros(learned_nodes)

        for j in range(learned_nodes):
            minimum = np.minimum(bot_input, learned_weights[j])
            choice_signal[j] = (np.sum(minimum[0]) + np.sum(minimum[1])) + (1 - self.Art.alpha) * \
                               (size - np.sum(learned_weights[j][0]) - np.sum(learned_weights[j][1]))

        # prediction with wta code
        # signal = [(j, choice_signal[j]) for j in range(learned_nodes)]
        # signal.sort(key=itemgetter(1), reverse=True)
        # wta_predict = self.Art.mid.crs_ltm[signal[0][0]]
        # self.TopArt(top_input)
        # label = self.Art.top.top_stm
        # comparison = wta_predict == label


        # prediction with distributed code
        self.DistArt(bot_input)
        dist_predict = self.Art.top.top_stm

        self.TopArt(top_input)
        top_predict = self.Art.top.top_stm
        comparison = dist_predict == top_predict

        if np.array(comparison).all() == True:
            self.Art.corr_pred.append(1)
        else:
            self.Art.incorr_pred.append(1)
